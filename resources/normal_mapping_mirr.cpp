...
unsigned int renderQuad(float sz);
...
void renderCube(Shader& shader, int height, int m, int n, unsigned int dMap, unsigned int nMap,
                unsigned int f_nMap, unsigned int f_dMap, glm::vec3 move_to);

unsigned int renderMirrors(float sz, Shader shader, int num, int n, unsigned int textureColorbuffer,
...
int main()
{
    ...
    // lighting info
    // -------------
    glm::vec3 lightPos(1.2f, 0.7f, 2.9f);
   ...
   // glBindVertexArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec3 room_left(-5.0f, 0.0f, 20.0f);

        renderCube(shader, 2,80,5,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, room_left);
     ...
     //   unsigned int mVAO = renderMirrors(shader, 10, 80, mirrorMap);
        ...
        renderCube(shader, 2,70,3,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, room_left);
  //      renderMirrors(shader, 10, 80, mirrorMap);
     //   renderCube(shader, 2,10,10,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, room_left);
        // draw the lamp object
        lightCubeShader.use();
        lightCubeShader.setMat4("projection", projection);
        ...
        renderMirrors(0.3, shader, 10, 80, textureColorbuffer, mirror_diffuseMap, mirror_normalMap);
...
unsigned int renderMirrors(Shader& shader, int num, int n, unsigned int mirrorMap){
...
//    model_b = glm::translate(model_b, move_to);
...
void renderCube(Shader& shader, int height, int m, int n, unsigned int dMap, unsigned int nMap,
    ...
    for(int k = 0; k < height; ++k){
        renderCubeLayer(shader, k, dMap, nMap, m, n, move_to);
    }
    ...
    for(int i = 0; i < m; ++i){
        ...
        //    model_cl = glm::translate(model_cl, move_to);
    ...
    for(int i = 0; i < m; ++i){
        for(int j = 0; j < n; ++j){
         ...
         //   model_fl = glm::translate(model_fl, move_to);
...
void _renderQuad(float sz)
{

    if(quadVAO == 0){
        // positions
        glm::vec3 pos1(-sz,  sz, 0.0f);
        glm::vec3 pos2(-sz, -sz, 0.0f);
        glm::vec3 pos3( sz, -sz, 0.0f);
        glm::vec3 pos4( sz,  sz, 0.0f);

        glm::vec3 pos5(-sz, 0.0f,  0.5f);
        glm::vec3 pos6(-sz, 0.0f, -0.5f);
        glm::vec3 pos7( sz, 0.0f, -0.5f);
        glm::vec3 pos8( sz, 0.0f,  0.5f);

        glm::vec3 pos9( -sz, 0.0f,  sz);
        glm::vec3 pos10(-sz, 0.0f, -sz);
        glm::vec3 pos11( sz, 0.0f, -sz);
        glm::vec3 pos12( sz, 0.0f,  sz);
                           ...
                           unsigned int mirror_diffuseMap, unsigned int mirror_normalMap)
{
    ...
    glm::mat4 model = glm::mat4(1.0f);
    glm::mat4 model_b = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    shader.setVec3("light.ambient", 5.5f, 5.5f, 5.5f);
    shader.setVec3("light.diffuse", 7.0f, 7.0f, 7.0f);
    shader.setVec3("light.specular", 15.0f, 15.0f, 15.0f);
    shader.setFloat("light.constant", 1.0f);
    shader.setFloat("light.linear", 0.09f);
