...
////    unsigned int mirror_diffuseMap;
...
////
////class Settings{
////    std::unique_ptr<CubeSts> cube;
////    std::unique_ptr<MirrorSts> mirror;
////    std::unique_ptr<LightSts> light;
////};

unsigned int renderRoom(Shader& shader, int height, int m, int n, unsigned int dMap, unsigned int nMap,
                ...
                Shader& lightShader, glm::mat4 view, glm::mat4 projection, float scale);

unsigned int renderCube(Shader& shader, int height, int m, int n, unsigned int dMap, unsigned int nMap,
                unsigned int f_nMap, unsigned int f_dMap, glm::vec3 move_to, float scale);
...
//    Shader shader("normal_map.vs", "normal_map.fs");
   ...
   // glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)(2 * sizeof(float)));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lastQuad += renderRoom(shader, 2,80,3,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, origin,
                               0.3, basicShader, num_mirrors, textureColorbuffer, door_outer_nMap, window_noborder_dMap, window_noborder_nMap, mirror_normalMap,
                               num_lights, lightCubeVAO, crystal_diffuseMap, crystal_normalMap, lightShader, view, projection, room_scale);
        lastQuad += renderRoom(shader, 2,20,20,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, move1,
                               0.3, shader, num_mirrors, textureColorbuffer, door_outer_nMap, window_noborder_dMap, window_noborder_nMap, mirror_normalMap,
                               num_lights, lightCubeVAO, crystal_diffuseMap, crystal_normalMap, lightShader, view, projection, room_scale);
        lastQuad += renderRoom(shader, 3,15,30,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, move2,
                               0.3, shader, num_mirrors, textureColorbuffer, door_outer_nMap, window_noborder_dMap, window_noborder_nMap, mirror_normalMap,
                               num_lights, lightCubeVAO, crystal_diffuseMap, crystal_normalMap, lightShader, view, projection, room_scale);
        lastQuad += renderRoom(shader, 2,10,20,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, move3,
                               0.3, shader, num_mirrors, textureColorbuffer, door_outer_nMap, window_noborder_dMap, window_noborder_nMap, mirror_normalMap,
                               num_lights, lightCubeVAO, crystal_diffuseMap, crystal_normalMap, lightShader, view, projection, room_scale);

        lastQuad += renderRoom(shader, 2,3,70,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, move4,
                               0.3, shader, num_mirrors, textureColorbuffer, door_outer_nMap, window_noborder_dMap, window_noborder_nMap, mirror_normalMap,
                               num_lights, lightCubeVAO, crystal_diffuseMap, crystal_normalMap, lightShader, view, projection, room_scale);

        lastQuad += renderRoom(shader, 2,30,3,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, move5,
                               0.3, shader, num_mirrors, textureColorbuffer, door_outer_nMap, window_noborder_dMap, window_noborder_nMap, mirror_normalMap,
                               num_lights, lightCubeVAO, crystal_diffuseMap, crystal_normalMap, lightShader, view, projection, room_scale);

       // renderCube(shader, 2,10,10,diffuseMap,normalMap,floor_normalMap,floor_diffuseMap, room_left);
     //   unsigned int mVAO = renderMirrors(shader, 10, 80, mirrorMap);
...
unsigned int renderRoom(Shader& shader, int height, int m, int n, unsigned int dMap, unsigned int nMap,
   ...
   // glStencilFunc(GL_ALWAYS, 1, 0xFF);
    ...
    lastQuad = renderCube(shader, height, m, n, dMap, nMap, f_nMap, f_dMap, move_to, scale);
...
unsigned int renderCubeLayer(Shader& shader, int layer, unsigned int diffuseMap, unsigned int normalMap,
                     int m, int n, glm::vec3 move_to, float scale)
{
    unsigned int lastQuad = 0;
...
unsigned int renderCube(Shader& shader, int height, int m, int n, unsigned int dMap, unsigned int nMap,
    ...
    for(int k = 0; k < height; ++k){
        renderCubeLayer(shader, k, dMap, nMap, m, n, move_to, scale);
    }
                 ...
                 glm::mat4 view, glm::mat4 projection)
{
    ...
    //renderCube(shader, 0.6, w, 0.3, mirror_diffuseMap, mirror_normalMap,
     ...
     renderCube(shader, 15, width, 1, inner_diffuseMap, inner_normalMap,
                inner_normalMap, inner_diffuseMap,  move_to, scale-border_scale);
    ...
    // move_to.z *= (1+border_scale);
     ...
     //move_to.z += (width*border_scale)/2;
     renderCube(normal_shader, 13, width, 1, outer_diffuseMap, outer_normalMap,
                outer_normalMap, outer_diffuseMap,  move_to, scale);
    // glBindVertexArray(0);
    // glStencilMask(0x00);
