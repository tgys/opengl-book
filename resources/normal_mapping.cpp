...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // light properties
        shader.setVec3("light.ambient", 5.0f, 5.0f, 5.0f);
...
void renderCube(Shader& shader)
{
    ...
    glm::mat4 model_b = glm::translate(model, glm::vec3(0.0f,0.0f,static_cast<float>(n)));
    model_b = glm::translate(model, glm::vec3(static_cast<float>(m)+1, 0.0f,0.0f));
    model_b = glm::rotate(model_b, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    model_b = glm::translate(model_b, glm::vec3(0.0f,0.0f,-static_cast<float>(n)));
   // renderQuad();
    for(int i = 0; i < m; ++i){
        model_f = glm::translate(model_f, glm::vec3(1.0f,0.0f,0.0f));
        shader.setMat4("model", model_f);
        renderQuad();

        model_b = glm::translate(model_b, glm::vec3(1.0f,0.0f,0.0f));
        shader.setMat4("model", model_b);
        renderQuad();
    }
    glm::mat4 model_l = glm::translate(model, glm::vec3(static_cast<float>(n),0.0f,0.0f));
    model_l = glm::rotate(model_l, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    model_l = glm::translate(model_l, glm::vec3(static_cast<float>(n)+0.5,0.0f, static_cast<float>(m)+1.5f));

    //renderQuad();
    for(int j = 0; j < n; ++j){
        model_l = glm::translate(model_l, glm::vec3(-1.0f,0.0f,0.0f));
        shader.setMat4("model", model_l);
        renderQuad();
    }
  //  glm::mat4 model_r = glm::translate(model_l, glm::vec3(0.0f,0.0f, static_cast<float>(m)));
    glm::mat4 model_r = glm::translate(model, glm::vec3(0.0f,0.0f, 1.0f));
    model_r = glm::translate(model_r, glm::vec3(static_cast<float>(n),0.0f, 0.0f));
   ...
   // model_r = glm::translate(model_r, glm::vec3(0.0f,0.0f, static_cast<float>(m)));
    model_r = glm::translate(model_r, glm::vec3(static_cast<float>(n)-0.5,0.0f, 1.5f));
   // model_r = glm::translate(model_r, glm::vec3(0.0f,0.0f, static_cast<float>(n)));
    //renderQuad();
    for(int j = 0; j < n; ++j){
        model_r = glm::translate(model_r, glm::vec3(-1.0f,0.0f,0.0f));
        shader.setMat4("model", model_r);
        renderQuad();
    }
...
//    renderQuad();
    //glm::mat4 model_l = glm::translate(model, glm::vec3(1.0f,0.0f,1.0f));
    //model_l = glm::rotate(model_l, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    //shader.setMat4("model", model_l);
    //renderQuad();

    //glm::mat4 model_r = glm::translate(model, glm::vec3(-1.0f,0.0f,1.0f));
    //model_r = glm::rotate(model_r, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    //shader.setMat4("model", model_r);
    //renderQuad();
