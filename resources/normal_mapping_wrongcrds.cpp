...
unsigned int renderRoom(RoomSettings& rsts, int height, int m, int n, glm::vec3 move_to, bool& pass_fb);
...
unsigned int _renderRoom(Shader& shader, int height, int m, int n, unsigned int lr_dMap, unsigned int lr_nMap,
                ...
                Shader& lightShader, glm::mat4 view, glm::mat4 projection, float scale, bool& pass_fb);
...
unsigned int renderCube(Shader& shader, int height, int m, int n, unsigned int lr_dMap, unsigned int lr_nMap, unsigned int fb_dMap, unsigned int fb_nMap,
                        unsigned int f_nMap, unsigned int f_dMap, unsigned int c_nMap, unsigned int c_dMap, glm::vec3 move_to, float scale, bool& pass_fb);
...
//    Shader shader("normal_map.vs", "normal_map.fs");
   ...
   // glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)(2 * sizeof(float)));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lastQuad += renderRoom(rsts,2,80,3,origin,pass_fb);
        lastQuad += renderRoom(rsts,2,20,20,move1,pass_fb);
        lastQuad += renderRoom(rsts, 3,15,30,move2,pass_fb);
        lastQuad += renderRoom(rsts, 2,10,20,move3,pass_fb);
        lastQuad += renderRoom(rsts, 2,3,70,move4,pass_fb);
        lastQuad += renderRoom(rsts, 2,30,3,move5,pass_fb);
...
unsigned int renderRoom(RoomSettings& rsts, int height, int m, int n, glm::vec3 move_to, bool& pass_fb){
    ...
    return _renderRoom(rsts.shader, height, m, n, rsts.lr_diffuseMap, rsts.lr_normalMap, rsts.fb_diffuseMap, rsts.fb_normalMap,
                       ...
                       rsts.lightShader, rsts.view, rsts.projection, rsts.room_scale, pass_fb);
}
...
unsigned int _renderRoom(Shader& shader, int height, int m, int n, unsigned int lr_dMap, unsigned int lr_nMap, unsigned int fb_dMap, unsigned int fb_nMap,
   ...
   // glStencilFunc(GL_ALWAYS, 1, 0xFF);
    ...
    if(pass_fb){
          glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
          glStencilMask(0x00);
          lastQuad = renderCube(shader, height, m, n, lr_dMap, lr_nMap, fb_dMap, fb_nMap, f_nMap, f_dMap, c_nMap, c_dMap, move_to, scale, pass_fb);
          glBindVertexArray(0);
          glStencilMask(0xFF);
...
unsigned int renderCubeLayer(Shader& shader, int layer, unsigned int lr_diffuseMap, unsigned int lr_normalMap, unsigned int fb_diffuseMap, unsigned int fb_normalMap,
                     int m, int n, glm::vec3 move_to, float scale, bool& pass_fb)
{
    unsigned int lastQuad = 0;
...
unsigned int renderCube(Shader& shader, int height, int m, int n, unsigned int lr_dMap, unsigned int lr_nMap, unsigned int fb_dMap, unsigned int fb_nMap,
    ...
    for(int k = 0; k < height; ++k){
        //glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        //glStencilMask(0x00);
        renderCubeLayer(shader, k, lr_dMap, lr_nMap, fb_dMap, fb_nMap, m, n, move_to, scale, pass_fb);
        //glBindVertexArray(0);
        //glStencilMask(0xFF);
        //glStencilFunc(GL_ALWAYS, 0, 0xFF);
        //glEnable(GL_DEPTH_TEST);
                 ...
                 glm::mat4 view, glm::mat4 projection)
{
    ...
    //renderCube(shader, 0.6, w, 0.3, mirror_diffuseMap, mirror_normalMap,
     ...
     renderCube(shader, 15, width, 1,
                ...
                outer_normalMap, outer_diffuseMap,  outer_normalMap, outer_diffuseMap, move_to, scale-border_scale, pass_fb);
     pass_fb = true;
     renderCube(shader, 15, width, 1,
                outer_diffuseMap, outer_normalMap, inner_diffuseMap, inner_normalMap,
                outer_normalMap, outer_diffuseMap,  outer_normalMap, outer_diffuseMap, move_to, scale-border_scale, pass_fb);
    ...
    // move_to.z *= (1+border_scale);
     ...
     renderCube(normal_shader, 13, width, 1, outer_diffuseMap, outer_normalMap, outer_diffuseMap, outer_normalMap,
                outer_normalMap, outer_diffuseMap, outer_normalMap, outer_diffuseMap, move_to, scale, pass_fb);
     pass_fb = true;
     renderCube(normal_shader, 13, width, 1, outer_diffuseMap, outer_normalMap, outer_diffuseMap, outer_normalMap,
                outer_normalMap, outer_diffuseMap, outer_normalMap, outer_diffuseMap, move_to, scale, pass_fb);
