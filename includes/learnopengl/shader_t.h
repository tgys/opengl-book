...
public:
    ...
    std::string get_path(const char* fn){
        std::string rp = "/home/tei/projects/LearnOpenGL/";
        for (std::filesystem::recursive_directory_iterator i(rp), end; i != end; ++i)
            if (!std::filesystem::is_directory(i->path())){
                if(i->path().filename() == fn){
           ...
           const char* tessControlPath = nullptr, const char* tessEvalPath = nullptr)
    {
        ...
        {
            // open files
            std::string v_path = get_path(vertexPath);
            std::string f_path = get_path(fragmentPath);
